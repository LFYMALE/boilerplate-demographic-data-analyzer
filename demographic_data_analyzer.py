...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = df[df['hours-per-week'] == min_work_hours]
    people_working_min_hours = df[df['hours-per-week'] == min_work_hours]
    ...
    salary_by_country = round((rich_people_by_country / country_counts) * 100, 1)

    highest_earning_country = salary_by_country.idxmax()


    highest_earning_country_percentage = round(salary_by_country.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = None
    high_salary_india = df[(df['salary'] == '>50K') & (df['native-country'] == 'India')]

    top_IN_occupation = high_salary_india['occupation'].mode()[0] if not high_salary_india.empty else "No data"
